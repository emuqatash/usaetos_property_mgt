...
<!--                        <ArrowNarrowRightIcon class="w-6 h-5 inline-block section-button-icon text-blue-700"/>-->
                    ...
                    <Tab title="Payment" class="space-y-6">
                        ...
                        <template v-if="tenantContractPaymentDetails?.data?.length">
                            <div class="-mx-4 mt-8 flow-root sm:mx-0">
                                <table class="min-w-full divide-y divide-gray-300">
                                    ...
                                    <tr class="border-b border-gray-200"
                                        v-for="(eachtenantContractPayment, index) in tenantContractPaymentDetails.data" :key="index"
                                        :class="{'bg-gray-100': selectedRow === eachtenantContractPayment.id,
                                        'divide-y divide-gray-10': selectedRows.includes(eachtenantContractPayment.id) }"
                                        @click="selectRow(eachtenantContractPayment.id)">
                                        <td class="flex-wrap lg:whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-0">
                                            {{ eachtenantContractPayment.payment_date }}</td>
                                        <td class="flex-wrap lg:whitespace-nowrap py-3 text-sm text-gray-500">{{ eachtenantContractPayment.payment_amount }}</td>
                                        <td class="relative whitespace-nowrap py-4 pr-4 text-right text-sm font-medium sm:pr-0" >
                                            <DotsVertical :eachRecord="eachtenantContractPayment.id" @submit-form="recordAction"
                                                          :allowDuplicate="true"/>
                                        </td>
                                    </tr>
                                    </tbody>
                                    ...
                                    <!--------------------pagination--------------------------->
                                    <Pagination :pagination="tenantContractPaymentDetails" />
                                    ...
                                    <ConfirmationModal
                                        ...
                                        @onCancel="closeModel"
                                        :show="modalActive"
                                        :message="'Are you sure you want to delete this Payment Detail ?'"
                        ...
                        <template v-else>
                                <EmptyState @page-Create-Active="createNewRecord" :title="'Payment Detail'"/>
                        </template>
                                    ...
                                    <div class="text-center">
                                        ...
                                        <div id="app" class="z-10">
                                            ...
                                            >
                                                ...
                                                >
                                                    <p class="text-md" :class="imageClass">Upload a file</p>
...
let form = useForm({
    ...
    // 'payment_date': props.tenantContracts ? props.tenantContracts.payment_date : '',
    // 'payment_amount': props.tenantContracts ? props.tenantContracts.payment_amount : '',

    'payment_date': props.tenantContracts.tenantContractPaymentDetails ? props.tenantContracts.tenantContractPaymentDetails.payment_date : '',
    'payment_amount': props.tenantContracts.tenantContractPaymentDetails ? props.tenantContracts.tenantContractPaymentDetails.payment_amount : '',
...
// tenant-contract-payment-detail section
const createNewRecord = () => {
    router.get(route('tenant-contract-payment-detail.createPaymentDetail', props.tenantContracts.id))
}
const deleteRecordConfirmed = (id) => {
    router.delete(route('tenant-contract-payment-detail.destroy', id))
...
const recordAction = (id, action) => {
    ...
    switch(action) {
        case 'viewRecord':
            router.get(route('tenant-contract-payment-detail.edit', id))
            break;
        case 'duplicateRecord':
            router.get(route('tenant-contract-payment-detail.duplicate', id))
            break;
        case 'deleteRecord':
